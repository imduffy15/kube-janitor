...
class TestDeleteNotification(unittest.TestCase):
    ...
    def test_add_notification_flag(self):
        ...
        self.assertEqual(self.mocked_resource.obj.get('metadata').get('annotations').get('janitor/notified'), 'yes')

    def test_add_notification_flag_dry_run(self):
        add_notification_flag(self.mocked_resource, dry_run=True)
        self.assertIsNone(self.mocked_resource.obj.get('metadata').get('annotations').get('janitor/notified', None))
    ...
    def test_handle_resource_ttl_annotation_with_notification_not_triggered(self, mocked_add_notification_flag, mocked_utcnow):
    ...
    def test_handle_resource_ttl_annotation_notification_event(self, mocked_create_event, mocked_utcnow):
        ...
        mocked_create_event.assert_called_with(resource, message, 'DeleteNotification', dry_run=True)

    @unittest.mock.patch('kube_janitor.janitor.utcnow', return_value=datetime.datetime.strptime('2019-03-11T11:10:09Z', '%Y-%m-%dT%H:%M:%SZ'))
    @unittest.mock.patch('kube_janitor.janitor.add_notification_flag')
    def test_handle_resource_expiry_annotation_with_notification_not_triggered(self, mocked_add_notification_flag, mocked_utcnow):
        # Resource was created: 2019-03-11T11:05:00Z
    ...
    @unittest.mock.patch('kube_janitor.janitor.utcnow', return_value=datetime.datetime.strptime('2019-03-11T11:12:09Z', '%Y-%m-%dT%H:%M:%SZ'))
    @unittest.mock.patch('kube_janitor.janitor.add_notification_flag')
    ...
    @unittest.mock.patch('kube_janitor.janitor.utcnow', return_value=datetime.datetime.strptime('2019-03-11T11:12:09Z', '%Y-%m-%dT%H:%M:%SZ'))
    @unittest.mock.patch('kube_janitor.janitor.create_event')
    def test_handle_resource_expiry_annotation_notification_event(self, mocked_create_event, mocked_utcnow):
        # Resource was created: 2019-03-11T11:05:00Z
        # Expire is set to: 2019-03-11T11:15:00Z
        # Current datetime is: 2019-03-11T11:10:09Z
        # Notification is 3 minutes: 180s. Has to notify after: 2019-03-11T11:12:00Z
        resource = MockedNamespace(None, {'metadata': {'name': 'foo', 'annotations': {
            'janitor/expires': '2019-03-11T11:15:00Z'}, 'creationTimestamp': '2019-03-11T11:05:00Z'}})
        delete_notification = 180
        handle_resource_on_expiry(resource, [], delete_notification, dry_run=True)
